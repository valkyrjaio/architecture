...
- **Cross-language change — mirror this in every port (Go, Java, Python,
  TypeScript) so the test trees stay 1:1.** Rename the reusable-test-double
  sub-namespace from `Classes` to `Fixtures`. "Fixtures" is the widely-understood
  term for reusable test doubles/sample classes; "Classes" is generic and reads
  oddly next to `Unit`/`Functional`/`Abstract`.
- PHP specifics:
    - Move `tests/Tests/Classes/` → `tests/Tests/Fixtures/` (keep the
      `Provider`/`Trait`/`Contract`/… concept subdivisions).
    - Rename namespace `Vendor\Tests\Classes\…` → `Vendor\Tests\Fixtures\…` and
      update every `use` import / reference across the suites and in `sindri`.
    - Update the `autoload-dev` PSR-4 mapping in `composer.json`.
    - Update the PHPArkitect rule(s) that assert the `Classes` location/naming.
...
- Also update shared docs once the rename lands: `TESTING_METHODOLOGY.md`
  (the test-tree table), the Java/PHP test-structure tables in `CLAUDE.md`, and
  the existing `Tests\Classes` mentions elsewhere in this file.
