...
- **PR titles:** the root and the type of the whole change, plus `(#123)` when
  ...
  type has to match any one branch commit. A branch carrying a `feat`, a
  `deprecate`, or a breaking commit takes `feat`, `deprecate`, or `!` on the
  title. The release bump reads only that type. Fill the PR template
