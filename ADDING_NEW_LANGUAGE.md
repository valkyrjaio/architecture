...
- **Commits:** `[Root] type: Imperative description.` (trailing period) — see
  [`COMMIT_CONVENTION.md`](COMMIT_CONVENTION.md). For adding a whole language's
  workflows the sibling history uses the **language root** (e.g. `[Go] feat:`);
...
- **PR titles:** the root and the type of the whole change, plus `(#123)` when
  ...
  type has to match any one branch commit. What the change does then fixes the
  type. The release bump reads the type on the title, and never the type on a
  branch commit. A change that adds a capability, marks API deprecated, or
  breaks a public contract takes `feat`, `deprecate`, or `!` on the title. Fill
  the PR template (Description, Types of changes, Changes — bold path — em
  dash — what changed).
