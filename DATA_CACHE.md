...
Cache data files are generated by `sindri` — a standalone tool that ships with the framework. The framework
...
# Application cache — generated by sindri
app/cache/
...
export class UserServiceProvider implements ServiceProviderContract {
    ...
    publishUserRepository(c: ContainerContract): void {
        c.setSingleton(
            UserRepositoryClass,
            new UserRepository(c.getSingleton(DatabaseClass))
        )
    }
}
```
...
| Go         | ❌ string const (bindings only) | ✅ interface methods | ✅                  | ❌ not yet built                                                                        |
| Python     | ✅ class object                 | ✅                   | ✅                  | ❌ not yet built                                                                        |
...
handler expression from the AST and prints that expression into the generated file. See [`BUILD_TOOL.md`](BUILD_TOOL.md)
for the way each language resolves a type name in that expression. See [`HANDLERS.md`](HANDLERS.md) for the handler
design.
