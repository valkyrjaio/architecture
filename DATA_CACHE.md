...
Data cache files are pre-generated source files containing fully resolved, statically defined route, listener, command,
and container binding data. They allow the framework to skip the bootstrap process entirely on cold starts — instead of
...
Without cache data files, every CGI request or lambda cold start pays the full bootstrap cost:

- Traverse the full provider tree (AppProvider → ComponentProviders → SubProviders)
...
At framework scale — Container, Http, Routing, Dispatcher, Event, CLI, Queue, Cache, Session, Auth and more — the
binding count is not small and the provider chain traversal has real cost. Cache data files reduce cold start cost to
near-zero by pre-computing all of the above into static data classes that are simply loaded.

**Worker mode** pays this cost once at startup and amortizes it across all requests — cache is an optimization.

**CGI / lambda mode** pays this cost on every single request or cold start — cache is a hard production requirement.

---

## The valkyrja-forge Tool

Cache data files are generated by `valkyrja-forge` — a standalone tool that ships with the framework. The framework
itself has zero AST or build dependency. All source extraction logic lives in the build tool.
...
written as text into the generated files. The compiler/runtime resolves them when the generated files are
...
The build tool reads the application config class and discovers everything else by walking the static provider tree:
...
convention — the build tool fails with a clear error if it encounters dynamic returns.

---

## Application Config — Build Tool Entry Point

No separate `valkyrja.yaml` file is needed. The application config class is already the authoritative list of component
providers — it is the build tool's entry point.
...
Provider class references must use `::class` / `.class` / class objects directly — never string constants from a
constants class. The build tool cannot follow constant references without executing code.
...
container bindings) are unaffected — they are never used in provider lists.

Pre-generated framework and third-party cache files are discovered by the build tool from conventional locations in the
vendor directory — no explicit listing needed.
...
This is **one merge of two pre-built structures** — not N merges of N provider outputs. The build tool does the
aggregation at generation time so the framework pays minimal cost at boot.
...
# Application cache — generated by valkyrja-forge
app/cache/
...
The build tool requires access to provider source files to extract bindings and handlers. This means:

- **PHP / Python** — source always present on disk via Composer / pip. No special requirement.
...
All five languages implement the same provider contract shape. The build tool reads the return values of these methods
directly from AST — no execution required.
...
reads the map from AST then reads each referenced method body.
...
Route providers. Two sources: annotated controller classes (PHP, Java, Python) or explicit route objects (all
languages). Go and TypeScript use explicit routes only.
...
All other languages follow the same pattern as `HttpRouteProviderContract` — annotated classes for PHP/Java/Python,
explicit definitions for Go/TypeScript.
...
Parameter objects appear as inline constructor calls — AST reads them as static literals:
...
The AST reads each `Parameter` constructor call as a node with literal string arguments:
...
1. AST extracts path string, parameter list, handler, method, and all route metadata
2. Build tool constructs a plain `ValkyrjaRoute` object from the extracted data — enough for the processor
3. Build tool passes the constructed route to `ProcessorContract::route()` — unchanged, same as runtime
4. Processor returns the route with compiled regex filled in
5. Build tool reads all pre-compiled data and writes the `HttpRoutingData` instance as source text

The build tool runs framework code (`ProcessorContract`) during cache generation. This is valid — the build tool is
compiled from the framework source and runs in the same language environment.
...
The build tool is a **source code generator** — it writes strings that are valid source code. It does not need to
instantiate, extend, or have access to application-defined custom route classes at generation time. It simply writes
their names as text.

This is exactly how every pre-compilation step works in every language:

- **Java annotation processor** — generates source files referencing application classes that don't exist yet when the
  processor runs. The compiler resolves them in the final compile pass.
- **Go code generation** — `go generate` writes `.go` files importing packages. `go build` resolves them.
- **TypeScript pre-step** — generates `.ts` files importing application types. `tsc` resolves them.
- **PHP build tool** — writes `.php` files extending application classes. PHP resolves them via autoloader at runtime.

A developer writing `extends AuthenticatedRoute` in their editor doesn't need `AuthenticatedRoute` compiled and
available to type the name. The build tool has the same freedom.

---

### Custom Route Child Classes

If a developer defines a custom route child class and returns it from `getRoutes()`:
...
The AST sees `AuthenticatedRoute` as a string — the class name of the constructor call. The build tool:

1. Constructs a plain `ValkyrjaRoute` from the extracted data for regex compilation via `ProcessorContract`
2. Writes `AuthenticatedRoute` by name into the generated `HttpRoutingData` source text — no instantiation needed

The build tool does NOT need `AuthenticatedRoute` compiled in. It just writes the name. The compiler/runtime resolves it
when the generated file is compiled/loaded alongside the application.
...
The build tool aggregates **all** HTTP routes from **every** `HttpRouteProvider` across all components and the
application into a single `AppHttpRoutingData` class. This is the one class the framework loads for all HTTP routing.
The route objects are written as source text using actual class names from the AST — the build tool does not need those
classes compiled in at generation time.
...
Parameter definitions follow the same simple literal rule as all other build tool-readable data:
...
The `publishers()` method returns a map of binding key to method reference. The build tool reads the map then resolves
each method reference and reads that method's body:
...
This works because both the map and the referenced method bodies are simple, static, readable via AST without execution.
...
Every language works without cache data files. This is guaranteed by the provider contract design:

- **PHP, Java, Python** — class references (`::class`, `.class`, class objects) allow the framework to instantiate
  providers and call methods directly at runtime
- **TypeScript** — constructor reference type (`Array<new () => Contract>`) allows direct instantiation and method calls
  at runtime — no string lookup, no registry needed
- **Go** — interface methods called directly on provider structs at runtime — no reflection needed
...
This means the build tool and cache generation infrastructure can be built incrementally — the framework ships and works
...
**The PHP CLI command is a known TODO** — it needs to be migrated to the valkyrja-forge tool approach before handler
...
|------------|---------------|--------------------------------------|------------------------------------------------------|
| PHP        | ✅ works       | ⚠️ CLI command exists but will break | Migrate to valkyrja-forge before handler logic ships |
| Java       | ✅ works       | ❌ not yet built                      | valkyrja-forge Java implementation pending           |
| Go         | ✅ works       | ❌ not yet built                      | valkyrja-forge Go implementation pending             |
| Python     | ✅ works       | ❌ not yet built                      | valkyrja-forge Python implementation pending         |
| TypeScript | ✅ works       | ❌ not yet built                      | valkyrja-forge TypeScript implementation pending     |
...
When designing the `ServiceProviderContract`, a natural question arose: could routes and listeners be expressed the same
way as container bindings — a `publishers()`-style map of identifier to handler method, with the build tool reading
method bodies directly from AST? This would eliminate `getRoutes()` / `getListeners()` entirely and unify all provider
patterns into a single map-based approach.

This was considered and rejected for a fundamental architectural reason.

**Container bindings are simple key→factory pairs.** The binding key IS the complete identity. The factory closure is
the only additional data needed. A complete cache entry can be generated from just the method body.

**Routes are multi-dimensional data structures.** A route carries HTTP method, path pattern, dynamic segment
constraints, regex compilation data, middleware chain, name/alias, parameter defaults, host constraints, and scheme
constraints — all in addition to the handler. The `HttpRoute::get('/users/{id}', handler)` call populates all of these
fields together as a complete object. Decomposing this into a key/method-body map loses all the metadata the router
needs to build its dispatcher trie, compile route regexes, and resolve middleware chains.

**Listeners have the same problem.** A listener carries event type binding, priority, and stop-propagation behavior
alongside the handler. A flat key/body map cannot express these.

This is why `getRoutes()` and `getListeners()` return complete object lists while `publishers()` uses a map — the
difference reflects a genuine architectural distinction. Routes and listeners are data; container bindings are mappings.

---

## Discussion Summary

The data cache file solution emerged from recognizing that at framework scale the provider chain traversal has real and
meaningful cost — not just routes but container bindings for every component the application uses. The original
assumption that container bindings are cheap to register was revised when considering a full application using Http,
Container, Event, CLI, Queue, Cache, Session, and Auth components each with their own service providers.

The provider tree discovery problem was solved by requiring provider list methods to return simple list literals
readable by AST without execution. This is a hard framework convention — the build tool cannot work with dynamic or
conditional provider lists. The tradeoff is an explicit constraint on how providers are written in exchange for
build-time static analysis.

A separate `valkyrja.yaml` file was initially proposed as the build tool entry point. This was superseded by recognizing
that the application config class already contains the authoritative list of component providers — the same class the
developer uses to create the application. The build tool reads the config class via AST, eliminating a duplicate source
of truth and a maintenance burden. The component provider constants class was also dropped — it would have allowed
developers to pass constant references into the config which the build tool cannot resolve statically without executing
code. Binding key constants files are unaffected.

The framework source shipping policy was established to solve the compiled language problem: Java providers compiled
into a jar have no readable source for the build tool. The solution — requiring -sources.jar for Java and .ts source for
TypeScript — is a framework policy decision that makes source a first-class artifact rather than an optional
convenience.

The per-package cache file structure with load order override was established to preserve the provider system's
extensibility. Rather than one monolithic cache file, each package generates its own cache files. The application always
loads last and can override any framework or package binding. This mirrors exactly how the provider system works at
runtime.
...
compile that was considered early in the design — no bootstrap binary is needed because the build tool can discover
everything via static AST analysis of source files.
