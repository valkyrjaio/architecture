...
Java provider contracts mirror the PHP reference implementation. Key
differences:

- `.class` used instead of `::class` for class references — compiler-verified
  type tokens
...
- Publisher methods have no `@Handler` annotation — the build tool reads method
  bodies directly from AST
...
Top-level aggregator. Returns lists of sub-providers by category. Build tool
reads return values directly from AST — must be simple list literals with no
conditional logic.
...
Container bindings provider. `publishers()` returns a map of class token to
publisher method reference. The build tool reads the map from AST, resolves each
method reference, and reads that method body directly — no `@Handler` annotation
...
HTTP route provider. Two sources of routes: annotated controller classes (
scanned for `@Handler`) and explicit route object definitions. Routes are data
structures — they carry method, path, middleware, constraints, and the handler
...
bindings — a map of route key to handler method reference, with the build tool
reading method bodies directly. This was rejected because routes are
...
fields together. Decomposing this into a key/method-body map would lose all
