...
**Cross-language change — mirror this in every port (Go, PHP, Python, TypeScript)
so the test trees stay 1:1.** Rename the reusable-test-double package from
...
- Move `io.valkyrja.classes.*` → `io.valkyrja.fixtures.*` (and the repo's own
  root, e.g. `io.sindri.classes.*` → `io.sindri.fixtures.*`) under
  `.github/ci/junit/src/test/java/`.
- Update every import / reference across the JUnit suites.
- Update the ArchUnit rule(s) and any JaCoCo include/exclude globs that name the
  `classes` package.
...
- Update the Java test-structure table in `CLAUDE.md` and `TESTING_METHODOLOGY.md`
